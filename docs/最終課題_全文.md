...
[Issue](https://github.com/sghh02/pokedex-android-kadai/issues) の `#1` と `#2` を進めるための手順です。
...
### 2. リポジトリを Fork して手元に置く

コードを書く前に、自分の作業場所を用意します。

1. 課題リポジトリのページ右上の **Fork** を押し、自分のアカウントにコピーする
2. Fork したリポジトリを clone する

```bash
git clone https://github.com/<自分のアカウント>/pokedex-android-kadai.git
cd pokedex-android-kadai
```

3. Issue `#1` 用のブランチを切る

```bash
git switch -c feature/1-setup
```

以降は Issue ごとにブランチを切り、**1 Issue = 1 プルリクエスト**で進めます。
`.gitignore` は用意済みなので、ビルド成果物は自動的に除外されます。

### 3. プロジェクトを作る

Android Studio の **New Project** は空のディレクトリにしか作れません。
別の場所に作ってから、clone したディレクトリへ移します。
...
2. 設定値
    ...
    - Location: 一時的な場所でよい（例: `~/tmp/Pokedex`）
3. 生成されたファイルを、clone したディレクトリの直下へ移す

```bash
# clone したディレクトリで実行する
rsync -a --exclude .git ~/tmp/Pokedex/ .
rm -rf ~/tmp/Pokedex
```

4. Android Studio で clone したディレクトリを **Open** し直し、ビルドが通ることを確認する
...
| 初回起動が終わらない | 151回の詳細取得を逐次実行している。同時実行数を制限した並列取得にする（[実装上の注意](04_実装仕様.md#5-pokeapi-の実装上の注意)） |
